...
def price_od(od, bid_prices, eps, policy=None):
    ...
    if policy:  # backend guard (tùy chọn)
        ...
        if final != proposed:
            decision = "blocked"
    return dict(
        od_id=int(od["od_id"]), seat_type=stype,
        opportunity_cost=float(round(oc, 0)), proposed_price=float(round(proposed, 0)),
        final_price=float(round(final, 0)), decision=decision,
        explanation=dict(bottleneck_segment=(int(bottleneck) if bottleneck is not None else None),
                         segment_pi={int(k): float(v) for k, v in seg_pis.items()},
                         elasticity=float(round(e, 3)), base_price=float(od["base_price"])),
    )
