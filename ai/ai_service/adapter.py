...
def load_history(seeds_dir: str) -> pd.DataFrame:
    ...
    bagg = b.groupby(GKEY).agg(bookings=("booked_price", "size"), shown_price=("booked_price", "mean")).reset_index()

    # --- nhu cầu bị cắt (sold_out) từ search_logs ---
    sl["service_date"] = pd.to_datetime(sl["service_date"])
    so = sl[sl["result"] == "sold_out"].groupby(GKEY).size().reset_index(name="soldout")

    # --- thuộc tính cố định mỗi OD vật lý ---
    phys = (
        od.groupby(["origin_station_id", "destination_station_id", "seat_type"])
        .agg(base_price=("base_price", "median"), distance_km=("distance_km", "median"))
        .reset_index()
    )
    ...
    h = grid.merge(bagg, on=GKEY, how="left").merge(so, on=GKEY, how="left")
    ...
    h["crosses_bottleneck"] = ((h["origin_station_id"] <= HUE_ID) & (h["destination_station_id"] >= DAN_ID)).astype(int)
