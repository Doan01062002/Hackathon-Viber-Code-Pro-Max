...
def _prep(df: pd.DataFrame) -> pd.DataFrame:
    """Chuẩn bị đặc trưng; thêm target = bookings + soldout khi có đủ cột (predict-only thì bỏ qua)."""
    d = _ensure(df)
    if "bookings" in df.columns and "soldout" in df.columns:
        d["target"] = (df["bookings"] + df["soldout"]).astype(float)
    return d
