...
def load_history(seeds_dir: str) -> pd.DataFrame:
    od = pd.read_csv(_seed_path(seeds_dir, "od_products.csv"))
    trips = pd.read_csv(_seed_path(seeds_dir, "trips.csv"))[["id", "service_date"]].rename(columns={"id": "trip_id"})
    bk = pd.read_csv(_seed_path(seeds_dir, "bookings.csv"))
    sl = pd.read_csv(_seed_path(seeds_dir, "search_logs.csv"))
    cal = pd.read_csv(_seed_path(seeds_dir, "calendar_features.csv"))
    with open(_seed_path(seeds_dir, "stations.json"), encoding="utf-8") as stations_file:
    ...
    cal["is_holiday"] = _b(cal["is_holiday"])
    cal["is_tet"] = _b(cal["is_tet"])
    ...
    vo = b.groupby("origin_station_id").size()
    vo = vo / vo.max()
    vd = b.groupby("destination_station_id").size()
    vd = vd / vd.max()
...
def load_bookings_leadtime(seeds_dir: str) -> pd.DataFrame:
    ...
    bk = pd.read_csv(_seed_path(seeds_dir, "bookings.csv"))
    od = pd.read_csv(_seed_path(seeds_dir, "od_products.csv"))[["id", "trip_id"]].rename(columns={"id": "od_product_id"})
    trips = pd.read_csv(_seed_path(seeds_dir, "trips.csv"))[["id", "service_date"]].rename(columns={"id": "trip_id"})
