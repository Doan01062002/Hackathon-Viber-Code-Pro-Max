...
async def test_create_booking_hold_success(client):
    ...
    try:
        ...
        response = await client.post("/api/v1/booking", json={"od_product_id": 1, "channel": "web"})
        ...
        if "booking_id" in locals():
            ...
            seg_rows = db.execute(text("SELECT segment_id FROM od_product_segments WHERE od_product_id = 1")).fetchall()
            for r in seg_rows:
                db.execute(
                    text("""
...
async def test_confirm_booking_success(client):
    ...
    try:
        ...
        response = await client.post("/api/v1/booking", json={"od_product_id": 1})
        assert response.status_code == 201
        ...
        status_row = db.execute(
            text("SELECT status, seat_id FROM bookings WHERE id = :bid"), {"bid": booking_id}
        ).fetchone()
