...
from backend.views.analytics_view import LegHeatmapResponse, ForecastResponse
...
async def get_forecast_standard(
    trip_id: int = Query(..., description="ID của chuyến tàu"),
    seat_type: Optional[str] = Query(None, description="Lọc theo loại chỗ (ngoi_mem hoặc giuong_nam_k6)"),
    service: AnalyticsService = Depends(get_analytics_service),
    db: Session = Depends(get_db),
) -> ForecastResponse:
