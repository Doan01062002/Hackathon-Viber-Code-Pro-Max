...
class AuditLogResponse(BaseModel):
    ...
    entity_id: str | None = Field(None, description="ID của đối tượng chịu tác động")
    before_data: dict[str, Any] | None = Field(None, description="Dữ liệu trước thay đổi")
    after_data: dict[str, Any] | None = Field(None, description="Dữ liệu sau thay đổi")
...
async def get_audit_logs(
    actor: str | None = Query(None, description="Lọc theo người thực hiện"),
    action: str | None = Query(None, description="Lọc theo hành động"),
    entity_type: str | None = Query(None, description="Lọc theo loại đối tượng"),
    ...
    try:
        logs = service.get_audit_logs(
            db=db, actor=actor, action=action, entity_type=entity_type, limit=limit, offset=offset
        )
