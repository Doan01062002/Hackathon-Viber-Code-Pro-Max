...
class BookingService:
    def create_booking_hold(self, request: BookingCreateRequest, db: Session) -> BookingResponse:
        ...
        if quota_row:
            ...
            booked_count_row = db.execute(
                text("SELECT COUNT(*) FROM bookings WHERE od_product_id = :od_product_id AND status IN ('held', 'confirmed')"),
                {"od_product_id": od_product_id}
            ).fetchone()
            booked_count = int(booked_count_row[0]) if booked_count_row else 0
            
            if booked_count >= quota_limit:
                raise ValueError(f"Sản phẩm OD (ID {od_product_id}) đã vượt quá hạn ngạch (quota) tối đa là {quota_limit} vé")
        ...
        lock_query = text("""
            ...
            FOR UPDATE
        """)
        locked_rows = db.execute(lock_query, {
        ...
        if request.quote_id is not None:
            ...
            quote_row = db.execute(
                text("SELECT final_price, expires_at, decision FROM price_quotes WHERE id = :quote_id AND od_product_id = :od_product_id"),
        ...
        res = db.execute(insert_booking_query, {
    ...
    def confirm_booking(self, booking_id: int, db: Session) -> BookingConfirmResponse:
        ...
        seat_row = db.execute(seat_query, {
            "trip_id": trip_id,
            "seat_type": seat_type,
            "od_product_id": od_product_id
        }).mappings().first()
    ...
    def release_expired_bookings(self, db: Session) -> int:
        ...
        for row in expired_rows:
            ...
            prod_row = db.execute(
                text("SELECT seat_type FROM od_products WHERE id = :od_product_id"),
                {"od_product_id": od_product_id}
            ).fetchone()
