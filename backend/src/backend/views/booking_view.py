...
class BookingCreateRequest(BaseModel):
    ...
    quote_id: int | None = Field(None, description="ID của báo giá đã tạo (nếu có)")
    channel: str | None = Field("web", description="Kênh bán vé")
...
class BookingResponse(BaseModel):
    ...
    od_product_id: int = Field(..., description="ID của sản phẩm OD")
    seat_id: int | None = Field(None, description="ID của ghế vật lý được gán (nếu có)")
    ...
    expires_at: str | None = Field(None, description="Thời điểm hết hạn giữ chỗ (ISO format)")
